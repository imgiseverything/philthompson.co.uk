...
(function () {
  ...
  window.PT.slides = {
    ...
    navigate: function () {
      ...
      self.container
        .querySelectorAll(".js-slides-nav__link")
        .forEach(function (element) {
          element.addEventListener("click", function (e) {
            e.preventDefault();
            $activeNav = e.currentTarget;
            $activeSlide = self.container.querySelector(
              '[data-slide-number="' +
                $activeNav.getAttribute("data-slide-id") +
                '"]'
            );
            ...
            if ($activeNav.classList.contains(classes.active) !== true) {
              ...
              self.container
                .querySelectorAll(".js-slide")
                .forEach(function (element) {
                  const links = element.querySelectorAll("a");
                  element.setAttribute("aria-hidden", "true");
                  // hide any links from tabbing
                  if (links) {
                    links.forEach((link) => {
              ...
              $nav
                .querySelectorAll("." + classes.active)
                .forEach(function (element) {
    ...
    buildNavigation: function () {
      ...
      self.slides.forEach(function (ignore, index) {
        ...
        html.body +=
          '<button type="button" class="ir js-slides-nav__link slides-nav__button ' +
          isActiveClass +
          '" aria-expanded="' +
          isAriaExpanded +
          '" aria-controls="slide' +
      ...
      self.container.innerHTML =
        html.container.replace("{{BODY}}", html.body) +
        self.container.innerHTML;
